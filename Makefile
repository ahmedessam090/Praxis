...
.PHONY: help install temporal-up temporal-down temporal-nuke worker ui migrate \
        makemigration golden test unit integration smoke lint typecheck format clean
...
ui: ## Run the Streamlit UI (needs temporal-up + a worker)
